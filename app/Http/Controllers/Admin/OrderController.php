   ...
   public function order_pathao(Request $request)
    {
        ...
        $orders_id_array = explode(',', $orders_id);
    ...
    // Fetch all products and shipping charges
    $products = Product::select('id','name','new_price','product_code')->where(['status'=>1])->get();
    $shippingcharge = ShippingCharge::where('status',1)->get();
    ...
    // Reset the cart before adding updated order items
    Cart::instance('pos_shopping')->destroy();

    // Set session for product discount and shipping charge
    Session::put('product_discount', $order->discount);
    Session::put('pos_shipping', $order->shipping_charge);

    // Fetch order details and add them to the cart
    ...
    // Fetch the current cart content
    $cartinfo  = Cart::instance('pos_shopping')->content();

    // Get subtotal from cart
    $subtotal = Cart::instance('pos_shopping')->subtotal();
    $subtotal = str_replace(',', '', $subtotal);  // Clean up the subtotal

    // Get shipping fee from session
    $shipping = Session::get('pos_shipping', 0);

    // Get total discount from session
    $total_discount = Session::get('pos_discount', 0) + Session::get('product_discount', 0);

    // Pass all data to the view
    return view('backEnd.order.edit', compact('products', 'cartinfo', 'shippingcharge', 'shippinginfo', 'order', 'subtotal', 'shipping', 'total_discount'));
}
    ...
    public function order_update(Request $request){
        ...
        if($exits_customer){
            ...
            $store->slug        = $request->name;
            $store->phone       = $request->phone;
            $store->admin_note = $request->admin_note;
            $store->password    = bcrypt($password);
            $store->verify      = 1;
            $store->status      = 'active';
            $store->save();
