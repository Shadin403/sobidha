              ...
              <li>
                ...
                <div class="collapse" id="sidebar-orders">
                  <ul class="nav-second-level">
                        @php
                $total_order = \App\Models\Order::get()->count();
                $new_order = \App\Models\Order::where('order_status', 'orders')->count();
              @endphp
                    <li>
                      <a href="{{route('admin.orders',['slug'=>'all'])}}"><i data-feather="file-plus"></i> All Order <span class="badge bg-danger rounded-fill noti-icon-badge pt-1 pb-1 ">{{$total_order}}</span> </a>
                    </li>
                    @foreach($orderstatus as $value)
                    <li>
                      <a href="{{route('admin.orders',['slug'=>$value->slug])}}"><i data-feather="file-plus"></i>{{$value->name}} <span class="badge bg-danger rounded-fill noti-icon-badge pt-1 pb-1 ">{{$new_order}}</span></a>
                    </li>
                    @endforeach
                  </ul>
                </div>
